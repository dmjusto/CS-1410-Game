...
class FADE_BLOCK(pygame.sprite.Sprite):
    ...
    def __init__(self, LEVEL, T_POS, T_NAME, T_SIZE, L_POS, DELAY):
        ...
        self.delayF = DELAY # Number of frames before first fade begins
        self.solidF = 75 # number of frames that block remains solid/transparent
        self.alpha = 254 # block tile's alpha
        self.alpha_dir = -4 # how quickly block fades
        self.time = 0
        self.counter = 0
...
###
    ...
    def update(self,bullets,levelF_group):
        ...
        if self.delayF == 0:
            ...
            if self.counter == 0:
                ...
                if self.alpha >= 255 or self.alpha <= 0:
                    self.alpha_dir *= -1
                    self.counter = self.solidF
                
            if self.alpha < 175:
                self.solid = False
            else:
                self.solid = True
                hitsList = pygame.sprite.groupcollide(bullets, levelF_group,True, False)
